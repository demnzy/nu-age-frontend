...
async def dashboard_view(page: ft.Page):
    ...
    # --- 1. THE "SOCKET" FOR OPTIMISTIC UI ---
        ...
        content=ft.Column(
            controls=[
                ft.Row(
                    ...
                    controls=[
                        ft.Text("Weekly Activity", weight=ft.FontWeight.BOLD, size=16, color=ft.Colors.ON_SURFACE),
                        ft.Icon(ft.Icons.ANALYTICS_OUTLINED, color=ft.Colors.PRIMARY, size=20),
                    ]
                ),
                chart_holder # The socket goes here
            ],
            spacing=6,
        ),
    )
    ...
    async def fetch_activity_stats():
        ...
        weekly_bar_chart = fch.BarChart(
            groups=[
                fch.BarChartGroup(
                    x=i, 
                    rods=[fch.BarChartRod(from_y=0, to_y=val, color=ft.Colors.PRIMARY, width=16, border_radius=4)]
                ) for i, val in enumerate(new_activity_data)
            ],
            ...
            horizontal_grid_lines=fch.ChartGridLines(
                color=ft.Colors.with_opacity(0.1, ft.Colors.ON_SURFACE), width=1, dash_pattern=[4, 4]
            ),
        ...
        chart_holder.content = weekly_bar_chart
        page.update()
