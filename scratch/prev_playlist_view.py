...
async def playlist_view(page: ft.Page, playlist_id: str, back_target: str = "/courses"):
    ...
    def make_course_enroll_handler(course_id: str, btn_ref):
        async def handler(e):
            if btn_ref.current:
                ...
                btn_ref.current.text = "Enrolling…"
                page.update()
    ...
    def build_main_content() -> ft.Control:
        ...
        def pill_badge(text: str, icon=None, bg=None, fg=None):
            controls = []
            if icon:
                controls.append(ft.Icon(icon, size=12, color=fg or UI_ACCENT))
            controls.append(ft.Text(text, size=11, color=fg or UI_ACCENT, weight=ft.FontWeight.W_600))
            return ft.Container(
        ...
        def meta_stat_item(icon, label: str, value: str):
            return ft.Container(
                ...
                content=ft.Row(
                    ...
                    tight=True,
                    controls=[
                        ft.Icon(icon, size=16, color=UI_ACCENT),
                        ft.Column(
                            spacing=1,
                            tight=True,
                            controls=[
        ...
        badges_row = ft.Row(
            ...
            controls=[
                ...
                pill_badge(org_name, icon=ft.Icons.BUSINESS_ROUNDED,
                           bg=ft.Colors.with_opacity(0.06, ft.Colors.ON_SURFACE), fg=ft.Colors.ON_SURFACE),
                pill_badge("Public Curriculum" if is_public else "Organisation Exclusive",
                           icon=ft.Icons.PUBLIC_ROUNDED if is_public else ft.Icons.LOCK_OUTLINE_ROUNDED,
                           bg=ft.Colors.with_opacity(0.06, ft.Colors.ON_SURFACE), fg=ft.Colors.ON_SURFACE),
                pill_badge("Sequential Roadmap", icon=ft.Icons.TIMELINE_ROUNDED,
                           bg=ft.Colors.with_opacity(0.06, ft.Colors.ON_SURFACE), fg=ft.Colors.ON_SURFACE),
            ],
        )
        ...
        hero_stats_strip = ft.Row(
            ...
            run_spacing=8,
            controls=[
                meta_stat_item(ft.Icons.STAR_ROUNDED, "Path Rating", f"{rating} ★"),
                meta_stat_item(ft.Icons.LAYERS_OUTLINED, "Courses", f"{total_courses} in Sequence"),
                meta_stat_item(ft.Icons.TIMELAPSE_ROUNDED, "Duration", f"~{est_hours} Hours"),
                meta_stat_item(ft.Icons.WORKSPACE_PREMIUM_OUTLINED, "Milestone", "Track Certificate"),
            ],
        )
        ...
        def checklist_item(text: str):
            return ft.Row(
                spacing=10,
                vertical_alignment=ft.CrossAxisAlignment.START,
                controls=[
                    ft.Icon(ft.Icons.CHECK_CIRCLE_OUTLINE_ROUNDED, size=16, color=UI_ACCENT),
                    ft.Text(text, size=12, color=ft.Colors.ON_SURFACE, expand=True, weight=ft.FontWeight.W_500),
                ],
            )

        whats_included_card = ft.Container(
            bgcolor=ft.Colors.SURFACE,
            ...
            content=ft.Column(
                ...
                controls=[
                    ft.Row(
                        spacing=8,
                        controls=[
                            ft.Icon(ft.Icons.VERIFIED_OUTLINED, color=UI_ACCENT, size=18),
                            ft.Text("Path Features & Benefits", size=15, weight=ft.FontWeight.W_700, color=ft.Colors.ON_SURFACE),
                        ],
                    ),
                    checklist_item(f"Curated step-by-step sequence of {total_courses} courses"),
                    checklist_item("Unified progression across all modules and lessons"),
                    checklist_item("Interactive self-study flashcards, quizzes & exam engine"),
                    checklist_item("Offline sync & multi-device learning on Desktop & Mobile"),
                    checklist_item("Official Nu-Age path completion badge"),
                ],
            ),
        )

        # Publisher / Organisation Card
        publisher_card = ft.Container(
            bgcolor=ft.Colors.SURFACE,
            border_radius=ft.BorderRadius.all(16),
            border=ft.Border.all(1, ft.Colors.with_opacity(0.08, ft.Colors.ON_SURFACE)),
            padding=ft.Padding.all(18),
            content=ft.Column(
                ...
                controls=[
                    ft.Row(
                        spacing=8,
                        controls=[
                            ft.Icon(ft.Icons.BUSINESS_ROUNDED, color=UI_ACCENT, size=18),
                            ft.Text("Curated By", size=14, weight=ft.FontWeight.W_700, color=ft.Colors.ON_SURFACE),
                        ],
                    ),
                    ft.Row(
                        spacing=12,
                        ...
                        controls=[
                            ft.Container(
                                ...
                                alignment=ft.Alignment.CENTER,
                                content=ft.Icon(ft.Icons.CORPORATE_FARE_ROUNDED, size=20, color=UI_ACCENT),
                            ),
                            ft.Column(
                                spacing=2,
                                expand=True,
                                controls=[
                                    ft.Text(org_name, size=14, weight=ft.FontWeight.BOLD, color=ft.Colors.ON_SURFACE),
                                    ft.Text("Official Learning Organisation", size=11, color=ft.Colors.ON_SURFACE_VARIANT),
                                ],
                            ),
                        ],
                    ),
